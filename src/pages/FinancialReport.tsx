...
const FinancialReport = () => {
  ...
  const { companyInfo, transactions } = useAppContext();
  ...
  const { totalRevenue, totalExpense, balance } = useMemo(() => {
    ...
    filteredTransactions.forEach(t => {
      ...
      if (t.type === 'despesa') exp += t.amount;
    });
    return { totalRevenue: rev, totalExpense: exp, balance: rev - exp };
  }, [filteredTransactions]);

  const accountsSummary = useMemo(() => {
    const summary: Record<string, { receitas: number, despesas: number }> = {};
    filteredTransactions.forEach(t => {
      const cat = t.category || 'Outros';
      if (!summary[cat]) summary[cat] = { receitas: 0, despesas: 0 };
      if (t.type === 'receita') summary[cat].receitas += t.amount;
      if (t.type === 'despesa') summary[cat].despesas += t.amount;
    });
    return summary;
  }, [filteredTransactions]);
  ...
  return (
    <div className="p-4 lg:p-8 max-w-5xl mx-auto">
      ...
      >
        ...
        {/* Detailed Accounts */}
        <div className="mb-8">
          <h3 className="text-lg font-bold uppercase border-b border-slate-300 pb-2 mb-4">3. Descrição das Contas Movimentadas</h3>
          {Object.keys(accountsSummary).length > 0 ? (
            <table className="w-full text-sm text-left border-collapse">
              <thead>
                <tr className="bg-slate-100 border-b border-slate-300">
                  <th className="py-3 px-4 font-bold text-slate-700 uppercase">Categoria / Conta</th>
                  <th className="py-3 px-4 font-bold text-slate-700 uppercase text-right">Entradas (Receitas)</th>
                  <th className="py-3 px-4 font-bold text-slate-700 uppercase text-right">Saídas (Despesas)</th>
                  <th className="py-3 px-4 font-bold text-slate-700 uppercase text-right">Saldo</th>
                </tr>
              </thead>
              <tbody>
                {Object.entries(accountsSummary).map(([category, data], index) => (
                  <tr key={index} className="border-b border-slate-200">
                    <td className="py-3 px-4 font-medium text-slate-800">{category}</td>
                    <td className="py-3 px-4 text-right text-green-700">{data.receitas.toLocaleString('pt-MZ', { style: 'currency', currency: 'MZN' })}</td>
                    <td className="py-3 px-4 text-right text-red-700">{data.despesas.toLocaleString('pt-MZ', { style: 'currency', currency: 'MZN' })}</td>
                    <td className={`py-3 px-4 text-right font-bold ${data.receitas - data.despesas >= 0 ? 'text-green-800' : 'text-red-800'}`}>
                      {(data.receitas - data.despesas).toLocaleString('pt-MZ', { style: 'currency', currency: 'MZN' })}
                    </td>
                  </tr>
                ))}
              </tbody>
