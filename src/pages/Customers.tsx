      ...
      <section className="bg-surface-container-lowest rounded-xl shadow-sm overflow-hidden">
        <div className="p-6 border-b border-outline-variant/10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex items-center gap-4 w-full md:w-auto">
              ...
              {showColumnMenu && (
                <div className="absolute right-0 top-full mt-2 w-48 bg-surface-container-lowest border border-outline-variant/20 rounded-xl shadow-lg z-10 py-2">
                  ...
                  <label className="flex items-center gap-3 px-4 py-2 hover:bg-surface-container-low cursor-pointer">
                    ...
                    <span className="text-sm text-on-surface">E-mail</span>
                  </label>
                  <label className="flex items-center gap-3 px-4 py-2 hover:bg-surface-container-low cursor-pointer">
            ...
            <span className="text-xs text-on-surface-variant whitespace-nowrap hidden sm:inline">Total: {filteredCustomers.length} clientes</span>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-container-low/50">
                ...
                {visibleColumns.document && <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-on-surface-variant">Documento</th>}
                {visibleColumns.status && <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-on-surface-variant">Status</th>}
              ...
              {filteredCustomers.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-on-surface-variant">
                    {customers.length === 0 ? 'Nenhum cliente cadastrado ainda.' : 'Nenhum cliente encontrado na busca.'}
                  </td>
                </tr>
              ) : (
