...
const Suppliers = () => {
  const { suppliers, addSupplier, deleteSupplier, updateSupplier } = useAppContext();
            ...
            <tbody className="divide-y divide-outline-variant/5">
              {suppliers.map((s) => (
                <tr key={s.id} className="hover:bg-surface-container-low/30 transition-colors group">
                  <td className="px-6 py-4 text-sm font-bold text-primary">{s.name}</td>
                  ...
                  <td className="px-6 py-4">
                    <span className="px-3 py-1 bg-surface-container text-[10px] font-bold uppercase tracking-wider rounded-full text-on-surface-variant">
                      {s.category}
                    </span>
                  </td>
                  <td className="px-6 py-4">
              ...
              {suppliers.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-sm text-on-surface-variant">Nenhum fornecedor cadastrado.</td>
                </tr>
              )}
            </tbody>
          </table>
